...
function _selection() {
  ...
  if (!arguments.hasOwnProperty('types')) {
    let aux = {};
    aux[arguments.length] = 'types';
    Object.assign(params, arguments, aux);
  }
  ...
  for (let i = 0; i < Object.keys(params).length; i++) {
    ...
    for (let j = 0; j < keys.length; j++) {
      if (is.hasOwnProperty(keys[j])) {
        Object.assign(is[keys[j]], aux[keys[j]]);
      }
      else {
        is[keys[j]] = aux[keys[j]];
      }
    }
  }
...
function initializeAPI(is) {

  let api = {};

  api['not'] = not.bind(this);
  api['all'] = all.bind(this);
  api['any'] = any.bind(this);

  let interfaces = Object.keys(api);

  for (let option in is) {

    if (typeof is[option] === 'function') {
      for (let i = 0; i < interfaces.length; ++i) {
        if (!is.hasOwnProperty(interfaces[i]) &&
          ((!is[option].api) || ((is[option].api) && is[option].api.includes(interfaces[i])))) {
          is[interfaces[i]] = {};
        }

        if (!is[option].api || is[option].api.includes(interfaces[i])) {
          is[interfaces[i]][option] = api[interfaces[i]](is[option]);
        }
      }
    }
  }
...
function _regexpr(is) {
  ...
  function regexpCheck(regexp, regexes) {
    is[regexp] = function (value) {
      return value != null && regexes[regexp].test(value);
    };
  }
...
function _environment(is) {
  ...
  is.touchDevice = function () {
    return !!document && ('ontouchstart' in window ||
      ('DocumentTouch' in window && window.document instanceof DocumentTouch));
  };
